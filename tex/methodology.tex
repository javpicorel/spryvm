...
Like most recent work on VM~\cite{basu:efficient, pham:increasing,
  pham:colt, bhattacharjee:large-reach, barr:spectlb,
  papadopoulou:prediction-based, saulsbury:recently-based}, we use a
combination of analytical models, trace-driven functional simulation,
and prototypes on real hardware.

\subsection{Workloads}

First, we evaluate the impact of set-associativity on page fault
rates. To truly capture these effects, we need to use an experimental
setup with full-system effects and long uptimes typical of real
systems. For this reason, our initial page fault rate studies -- like
other recent work on VM accelerators \cite{picorel:near-memory} -- are
performed on real CPU systems as opposed to simulation. To
realistically stress our set-associative VM implemented in Linux, we
use several popular workloads, shown in
Table~\ref{table:workload}. These workloads perform complex
allocation/deallocation operations and fragment memory, stressing the
concept of set-associative virtual memory.

After establishing that set-associative VM does not increase page
fault rates significantly, we use cycle-accurate software simulation
to quantify the benefits of SpryVM. For these studies, we use
memory-intensive workloads with dynamic data structure traversal
similar to prior work ~\cite{picorel:near-memory,
  haria:devirtualizing, kocberber:meet, hsieh:accelerating}. We choose
these workloads because of their minimal data locality and
instruction-level parallelism, stressing conventional general-purpose
CPU architectures and hence favoring custom hardware. In our case, we
evaluate a set of data-structure traversals present in
ASCYLIB~\cite{david:asynchronized}, which contains state-of-the-art
implementations of hash tables, binary trees, and skip lists. Like
recent work~\cite{picorel:near-memory}, we present results for four
representative implementations.

\subsection{TLB Studies}

We collect memory traces using Pin~\cite{luk:pin} for the data
structure traversals. The traces contain the number of memory
operations executed in 1B instructions, which corresponds to the
execution of a few seconds, more than enough for the TLB sensitivity
experiments of at most a few thousand entries. We use the traces to
probe a set-associative TLB structure.
