...
first probed by the accelerator it resides in. Consider the case of a
TLB miss in an accelerator without caches residing close to memory, as
...
because of the page table structure or the presence of perfect MMU caches, the entire page
...
constitutes the translation path.

Upon successful translation, the accelerator uses the PTE to calculate
the physical address and issues the data fetch. A set of NoC and off-chip
...
As Figure~\ref{fig:pagewalk_base} shows, conventional address
...
calculate the physical memory region that a virtual page maps to. 
This can be accomplished using a simple hash function (e.g., as simple as using
a subset of the address bits) and the request can be routed to the particular region. This means that rather than
...
lookup. Once the PTE arrives from DRAM and the
