...
We propose DTRIM, a set-associative VM architecture for large heterogeneous systems.
...
DTRIM to partition the memory space and delegate the address translation to the
memory partition that holds the data, where memory-side per-partition TLBs and MMUs
can locally translate addresses at lower latency and with higher efficiency. In doing so, DTRIM 
significantly improves TLB hit rates and breaks page walk-data fetch serialization upon
TLB misses. DTRIM achieves within 1.2\% and 0.6\% of ideal translation in scenarios 
where working sets are memory-resident and exceed the available memory capacity,
respectively. Finally, we implement DTRIM in stock Linux, showing
